...
namespace HoloEvac_Micron
{
    public partial class PositionRecorderForm : Form
    {
        ...
        String skullMarkerName = "Skull1";
        String toolMarkerName = "Tool1";

        double[,] holoMarker = new double[4, 4];
        double[,] skullMarker = new double[4, 4];
        double[,] toolMarker = new double[4, 4];

        // plug in calibration matrix 
        double[,] calib = new double[4, 4] { { -0.006602, -0.00004307, 0.00013566, 0.18651107 }, { -0.00001732, 0.00649731, 0.00044547, -0.81390249 }, { -0.00034853, 0.0000621, -0.00653838, 0.4093864 }, { 0, -0, -0, 1 } };
        //End added by huimei
        ...
        private void AddRecordSingleFile()
        {
            ...
            while (i < markers.GetIdentifiedMarkers(null).Count)
            {
                ...
                for (int b = 0; b < PoseRecords[0, PoseRecordsCount].TemplateNames.Count; b++)
                {
                    ...
                    else if (templateName.Equals(skullMarkerName))
                    {
                        skullMarker = this.combineMatrix(PoseRecords[0, PoseRecordsCount].MarkerXforms[b].ShiftVector, PoseRecords[0, PoseRecordsCount].MarkerXforms[b].RotMat);

                        skullFlag = 1;
                    }
                    else if (templateName.Equals(toolMarkerName))
                    {
                        toolMarker = this.combineMatrix(PoseRecords[0, PoseRecordsCount].MarkerXforms[b].ShiftVector, PoseRecords[0, PoseRecordsCount].MarkerXforms[b].RotMat);

                        toolFlag = 1;
                    }

                    var line = new StringBuilder();
                    if (holoFlag == 1 && skullFlag == 1 && toolFlag == 1)
                    {
                        ...
                        for (int l = 0; l < skullMarker.GetLength(0); l++)
                        {
                            for (int m = 0; m < skullMarker.GetLength(1); m++)
                            {
                                line.Append(skullMarker[l, m]);
                                line.Append(",");
                            }
                        }
                        for (int l = 0; l < toolMarker.GetLength(0); l++)
                        {
                            for (int m = 0; m < toolMarker.GetLength(1); m++)
                            {
                                line.Append(toolMarker[l, m]);
                                line.Append(",");
                            }
                        }
                        log.Debug(line.ToString());

                        this.processData(holoMarker, skullMarker, toolMarker);
        ...
        //Added by huimei
        //TODO: add check if skull position changed for more than 3mm
        private void processData(double[,] holoMarker, double[,] skullMarker, double[,] toolMarker)
        {
            if (holoMarker != null && skullMarker != null && toolMarker != null && calib != null)
            {
                if (holoMarker.GetLength(0) == 4 && holoMarker.GetLength(1) == 4 && skullMarker.GetLength(0) == 4 && skullMarker.GetLength(1) == 4 && toolMarker.GetLength(0) == 4 && toolMarker.GetLength(1) == 4)
                {
                    if (calib.GetLength(0) == 4 && calib.GetLength(1) == 4)
                    {
                        String skullString = "";
                        String toolString = "";
                        ...
                        Matrix<Double> tmpSkull = inverseHololens.Multiply(DenseMatrix.OfArray(skullMarker));
                        Matrix<Double> tmpTool = inverseHololens.Multiply(DenseMatrix.OfArray(toolMarker));

                        double[,] skullTranslatedPosition = new double[4, 1];
                        double[,] toolTranslatedPosition = new double[4, 1];

                        for (int i = 0; i < 4; i++)
                        {
                            skullTranslatedPosition[i, 0] = tmpSkull[i, 3];
                            toolTranslatedPosition[i, 0] = tmpTool[i, 3];
                        }

                        if (tmpSkull != null && tmpTool != null)
                        {
                            Matrix<Double> calibratedSkullPosition = DenseMatrix.OfArray(calib).Multiply(DenseMatrix.OfArray(skullTranslatedPosition));
                            Matrix<Double> calibratedToolPosition = DenseMatrix.OfArray(calib).Multiply(DenseMatrix.OfArray(toolTranslatedPosition));

                            double[,] skullFinal = new double[4, 4];
                            double[,] toolFinal = new double[4, 4];
                            for (int i = 0; i < 4; i++)
                            {
                                for (int j = 0; j < 4; j++)
                                {
                                    if (i < 3)
                                    {
                                        if (j == 3)
                                        {
                                            skullFinal[i, j] = calibratedSkullPosition[i, 0] * (1 / calibratedSkullPosition[3, 0]);
                                            toolFinal[i, j] = calibratedToolPosition[i, 0] * (1 / calibratedToolPosition[3, 0]);
                                        }
                                        else
                                        {
                                            skullFinal[i, j] = tmpSkull[i, j];
                                            toolFinal[i, j] = tmpTool[i, j];
                                        }
                                    }
                                    else
                                    {
                            ...
                            skullString = matrixToString(skullFinal);
                            toolString = matrixToString(toolFinal);

                            if (skullString.Trim() != null && toolString.Trim() != null)
                            {
                                final.Append(skullString);
                                final.Append(";");
                                final.Append(toolString);

                                MainForm.SendMessage(final.ToString());
                            }
                            else
                            {
                                MainForm.SendMessage(toolString);
                            }
                        }
                    }
                }
