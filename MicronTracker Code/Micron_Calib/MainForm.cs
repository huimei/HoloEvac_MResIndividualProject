...
namespace MicronTrackerCalibration
{
    public partial class MainForm : Form
    {
        ...
        private String HoloLensIP = "192.168.1.4";
        ...
        private void MainForm_Load(System.Object sender, System.EventArgs e)
        {
            ...
            //Set up persistent properties file access
            log.Info("Start setting up properties");
            ...
            log.Info("End setting up properties");

            //Load the marker templates from files
            log.Info("Start loading templates from files");
            ...
            log.Info("End loading templates from files");

            //Set initial processing/display settings from the UI controls
            log.Info("Start set initial processing/display settings from the UI controls");
            markers.AutoAdjustCam2CamRegistration = mnuCamCamRegistration.Checked;
            IsShowingDistancesAndAngles = mnuDistancesAngles.Checked;
            IsShowingMarkersPositions = mnuMarkersPositions.Checked;
            IsShowingMarkersPositionsAtRefSpace = mnuPositionsAtRefSpace.Checked;
            ...
            log.Info("End set initial processing/display settings from the UI controls");

            //Locate and set up the camera
            log.Info("Start locate and set up the camera");
            if ((!SetupCameras()))
            {
            ...
            log.Info("End locate and set up the camera");

            //Start added by huimei
            frmpositionrecorder = new PositionRecorderForm(this);
            ShowAuxiliaryForm(frmpositionrecorder, false);
        ...
        private bool SetupCameras()
        {
            ...
            {
                log.Info("if (mCameras.Count == 0 || !string.IsNullOrEmpty(ErrStr))");
        ...
        {
            log.Info("In SelectCamera, where index = " + index);
        ...
        public void mmTimer_Tick(System.Object sender, System.EventArgs e)
        {
            ...
            if (client.Connected)
            {
                ...
                {
                    String tmpHolder = null;

                    byte[] buffer = new byte[client.ReceiveBufferSize];
                    int bytesRead = netstream.Read(buffer, 0, client.ReceiveBufferSize);

                    string dataReceived = Encoding.ASCII.GetString(buffer, 0, bytesRead);
                    log.Debug("Received : " + dataReceived);

                    if (!dataReceived.Equals(tmpHolder))
                    {
                        this.PosRecorder();
                    }
                }
        ...
        {
            client.Connect(HoloLensIP, portNum);
        ...
        private void ToolStripCapture_Click(object sender, EventArgs e)
        {
            ...
            if (CaptureEnabled)
            {
                ToolStripCapture.Image = MicronTrackerCalibration.Properties.Resources.start_icon;
            }
            else
            {
                ToolStripCapture.Image = MicronTrackerCalibration.Properties.Resources.Standby_icon;
            }
        }
